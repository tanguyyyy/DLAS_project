...
available_languages=["fr-FR","en-EN","es-ES"]
...
class UI(QDialog):
    ...
    def record_one(self, sample_rate, duration):
        ...
        sd.stop()
        try:
            wav = sd.rec(duration * sample_rate, sample_rate, 1,device=3)
        except Exception as e:
            print(e)
    ...
    def __init__(self):
        ...
        self.selected_language.addItems(available_languages)
        gen_layout.addWidget(QLabel("<b>Select language:<b>"), stretch=1)
        gen_layout.addWidget(self.selected_language, stretch=1)

        ## Generation
        #self.text_prompt = QPlainTextEdit(default_text)
        self.original_text_title = QLabel(40*"-"+"<b>Original Text<b>"+40*"-")
        self.original_text = QLabel("Here will be displayed the original text")
        #self.original_text.setText("Here will be displayed the original text")
        gen_layout.addWidget(self.original_text_title, stretch=1)
        gen_layout.addWidget(self.original_text, stretch=1)

        


        self.text_prompt_title = QLabel(34*"-"+"<b>Translated text (English)<b>"+34*"-")
        self.text_prompt = QLineEdit("Here is the speech translated with respect to the chosen embedding.")
 
        gen_layout.addWidget(self.text_prompt_title, stretch=1)
        gen_layout.addWidget(self.text_prompt, stretch=1)

        self.translate_button = QPushButton("Speech to text and translate")
        gen_layout.addWidget(self.translate_button)
        ...
        browser_layout.addWidget(self.log_window)
        self.logs = []
