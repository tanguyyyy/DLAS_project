...
class Toolbox:
    ...
    def setup_events(self):
        ...
        func = lambda: self.speech2text()
        self.ui.translate_button.clicked.connect(func)
    ...
    def synthesize(self):
        ...
        texts = self.ui.text_prompt.text().split("\n")
        #print(f"utterance: {self.ui.selected_utterance}")
        embed = self.ui.selected_utterance.embed    #Numpy array
    ...
    def speech2text(self):
        self.ui.original_text.setText("Proceeding to text to speech...")
        speech = self.ui.selected_utterance.wav
        sr = encoder.sampling_rate
        sf.write("current.wav",speech,sr)

        r = speech_recognition.Recognizer()
        with speech_recognition.AudioFile("current.wav") as source:
            # listen for the data (load audio to memory)
            audio_data = r.record(source)        
        try:
            text = r.recognize_google(audio_data,language=self.ui.selected_language.currentText())
        except:
            text = "WARNING: Speech to text failed. Please check the selected language"
        maxsize=80
        if len(text)>maxsize:
            N = len(text)//maxsize
            for n in range(1,N+1):
                c = n*maxsize
                while text[c] != " " and c<len(text)-1:
                    c += 1
                text =f"""{text[:c]}\n{text[c+1:]}"""
        self.ui.original_text.setText(text)
        self.ui.generate_button.setDisabled(False)
        self.ui.synthesize_button.setDisabled(False)
        self.translate()

    def translate(self):
        text = self.ui.original_text.text()
        if text == "WARNING: Speech to text failed. Please check the selected language":
            return None
        ...
        self.ui.text_prompt.setText(translated_text)
